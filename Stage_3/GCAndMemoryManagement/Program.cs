...
namespace GCAndMemoryManagement
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                using (StreamWriter file = textFile.CreateTextFileW(ConfigurationManager.AppSettings["path"], "Lucy.txt", out memory))
                {
            ...
            using (TextFiles textFile = new TextFiles())
            {
                using (StreamReader file = textFile.CreateTextFileR(ConfigurationManager.AppSettings["path"] + "\\Lucy.txt", out memory))
                {
